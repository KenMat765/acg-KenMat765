  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = hit0_brdf * hit0_normal.dot(hit0_refl) * hit1_rad / hit0_pdf / nsample; // write some code
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float rad = w_light * hit0_brdf * hit0_normal.dot(hit0_refl) * hit1_rad / hit0_pdf_light_sample / num_half_sample; // write some code
      ...
      for (int isample = 0; isample < nsample / 2; ++isample) {
        ...
        float rad = w_brdf * hit0_brdf * hit0_normal.dot(hit0_refl) * hit1_rad / hit0_pdf_brdf_sample / num_half_sample; // write some code
        img_mis[ih * img_width + iw] += rad;
      }
    }
  }
